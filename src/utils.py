...
def load_file(file_url):
    ...
    -- file_url: расположение JSON-файла
    Возвращает словарь с банковскими операциями
    '''
...
def make_operations(operations: list):
    ...
    def check_wallet(where: str):
        ...
        Возвращает зашифрованный кошелек
        '''
        wallet = "Неизвестно"
        try:
            card = inf[f'{where}']
...
def get_all_operations(operations: list):
    ...
    Возвращает данные со всеми отсортированными операциями
    '''
    information = ''
...
def get_executed_five(operations: list):
    ...
    -- operations: список экземпляров класса Operations
    Возвращает данные с пятью последними успешными операциями
    '''
